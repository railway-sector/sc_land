...
}`)}function Kt(s){s.code.add(S`vec4 unpackColor(uvec4 packedGaussian) {
vec4 color;
...
}`)}function Yt(s){s.code.add(S`mat3 quaternionToRotationMatrix(vec4 q) {
float x2 = q.x + q.x;
float y2 = q.y + q.y;
float z2 = q.z + q.z;
float xx = x2 * q.x;
...
}`)}class ue extends oe{constructor(){super(...arguments),this.totalGaussians=-1,this.focalLength=-1,this.minSplatRadius=-1,this.minSplatOpacity=-1,this.tanFov=Qe,this.cameraDelta=pe,this.cameraPos8k=pe}}function Be(s){const e=new le;e.varyings.add("vColor","vec4"),e.varyings.add("conicOpacity","vec4"),e.varyings.add("depth","float"),e.varyings.add("gaussianCenterScreenPos","vec2"),e.varyings.add("fragScreenPos","vec2"),e.outputs.add("fragColor","vec4",0),e.vertex.uniforms.add(new fe("splatOrderTexture",a=>a.splatOrder),new fe("splatAtlasTexture",a=>a.splatAtlas),new Xe("numSplats",a=>a.totalGaussians),new Y("focalLength",a=>a.focalLength),new Y("minSplatRadius",a=>a.minSplatRadius),new Y("minSplatOpacity",a=>a.minSplatOpacity),new et("tanFov",a=>a.tanFov),new me("cameraDelta",a=>a.cameraDelta),new me("cameraPos8k",a=>a.cameraPos8k),new _e("fullWidth",({camera:a})=>a.viewport[2]),new _e("fullHeight",({camera:a})=>a.viewport[3]),new ve("proj",a=>a.camera.projectionMatrix),new ve("view",a=>a.camera.viewMatrix),new tt("cameraPosition",a=>a.camera.eye)),e.vertex.include(Kt),e.vertex.include(Yt),e.vertex.include(Jt),e.include(at,s),e.vertex.code.add(S`float ndcToPixel(float ndcCoord, float screenSize) {
