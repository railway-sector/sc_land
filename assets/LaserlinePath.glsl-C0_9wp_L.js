import{ar as o,jj as pe,aC as T,qT as ue,xa as fe,rk as ge,lf as _e,bf as X,jR as Q,K6 as me,K3 as Pe,KA as ve,Ig as f,Jp as w,K8 as be,Ih as z,Ie as r,gU as U,Ac as Z,K5 as we,J7 as S,bg as $,gD as N,KB as Ve,az as Y,Jn as ee,IP as C,x8 as De,kg as m,aI as P,KC as te,aw as xe,gM as ie,kM as Ee,k1 as ye,kV as Le,bh as Se,jg as ne,aq as se,kK as Ce,x5 as Me,Ii as ae,Ij as re,U as le,Kc as Ae,I6 as $e,aE as W,kW as O,K4 as Re,Ag as Te,IN as Ie,Jx as Oe,Id as F,et as Ue,KD as qe,_ as u,Ix as D,Kd as je,Ke as H,KE as ze,i5 as Ne,KF as R,KG as We,kQ as Fe,z2 as He,kS as Be,kT as ke,kU as Ke,kX as Ge,e as M,g as Je,KH as Xe,em as B}from"./index-BrQW8HOS.js";import{t as Qe}from"./VisualElement-BSXziD11.js";let k=class{constructor(e){this._renderCoordsHelper=e,this._origin=o(),this._dirty=!1,this._count=0}set vertices(e){const n=pe(3*e.length);let i=0;for(const s of e)n[i++]=s[0],n[i++]=s[1],n[i++]=s[2];this.buffers=[n]}set buffers(e){if(this._buffers=e,this._buffers.length>0){const n=this._buffers[0],i=3*Math.floor(n.length/3/2);T(this._origin,n[i],n[i+1],n[i+2])}else T(this._origin,0,0,0);this._dirty=!0}get origin(){return this._origin}draw(e){const n=this._ensureVAO(e);n!=null&&(e.bindVAO(n),e.drawArrays(ue.TRIANGLES,0,this._count))}dispose(){this._vao!=null&&this._vao.dispose()}get _layout(){return this._renderCoordsHelper.viewingMode===1?Ye:et}_ensureVAO(e){return this._buffers==null?null:(this._vao??=this._createVAO(e,this._buffers),this._ensureVertexData(this._vao,this._buffers),this._vao)}_createVAO(e,n){const i=this._createDataBuffer(n);return this._dirty=!1,new fe(e,new ge(e,_e(this._layout),i))}_ensureVertexData(e,n){if(!this._dirty)return;const i=this._createDataBuffer(n);e.buffer()?.setData(i),this._dirty=!1}_createDataBuffer(e){const n=e.reduce((g,c)=>g+K(c),0);this._count=n;const i=this._layout.createBuffer(n),s=this._origin;let a=0,l=0;const d="startUp"in i?this._setUpVectors.bind(this,i):void 0;for(const g of e){for(let c=0;c<g.length;c+=3){const L=T(G,g[c],g[c+1],g[c+2]);c===0?l=this._renderCoordsHelper.getAltitude(L):this._renderCoordsHelper.setAltitude(L,l);const _=a+2*c;d?.(c,_,g,L);const j=X(G,L,s);if(c<g.length-3){for(let v=0;v<6;v++)i.start.setVec(_+v,j);i.extrude.setValues(_,0,-1),i.extrude.setValues(_+1,1,-1),i.extrude.setValues(_+2,1,1),i.extrude.setValues(_+3,0,-1),i.extrude.setValues(_+4,1,1),i.extrude.setValues(_+5,0,1)}if(c>0)for(let v=-6;v<0;v++)i.end.setVec(_+v,j)}a+=K(g)}return i.buffer}_setUpVectors(e,n,i,s,a){const l=this._renderCoordsHelper.worldUpAtPosition(a,Ze);if(n<s.length-3)for(let d=0;d<6;d++)e.startUp.setVec(i+d,l);if(n>0)for(let d=-6;d<0;d++)e.endUp.setVec(i+d,l)}};function K(t){return 3*(2*(t.length/3-1))}const Ze=o(),G=o(),Ye=Q().vec3f("start").vec3f("end").vec2f("extrude").vec3f("startUp").vec3f("endUp"),et=Q().vec3f("start").vec3f("end").vec2f("extrude");function oe(t,e){const n=t.fragment;n.include(me),t.include(Pe),n.include(ve),n.uniforms.add(new f("globalAlpha",i=>i.globalAlpha),new w("glowColor",i=>i.glowColor),new f("glowWidth",(i,s)=>i.glowWidth*s.camera.pixelRatio),new f("glowFalloff",i=>i.glowFalloff),new w("innerColor",i=>i.innerColor),new f("innerWidth",(i,s)=>i.innerWidth*s.camera.pixelRatio),new be("depthMap",i=>i.depth?.attachment),new z("normalMap",i=>i.normals)),n.code.add(r`vec4 premultipliedColor(vec3 rgb, float alpha) {
