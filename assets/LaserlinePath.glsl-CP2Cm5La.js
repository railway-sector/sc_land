import{cm as o,cg as pe,cb as T,km as ue,sp as fe,kk as ge,je as _e,gx as J,g0 as K,G1 as me,F_ as ve,Gu as Pe,E4 as f,Fe as w,G3 as be,E5 as z,E2 as r,eP as I,v9 as Y,G0 as we,EY as S,gW as $,hg as N,Gv as Ve,my as Z,Fc as ee,ED as C,sm as De,gy as m,gz as v,Gw as te,sn as xe,kK as ie,hf as Ee,gb as ye,hu as Le,gV as Se,cc as ne,fl as se,hd as Ce,sj as Me,E6 as ae,E7 as re,_ as le,G7 as Ae,DW as $e,dA as W,hv as U,F$ as Re,vo as Te,EB as Oe,Fm as Ue,E1 as F,dJ as Ie,Gx as qe,n as u,El as D,G8 as je,G9 as G,Gy as ze,lc as Ne,Gz as R,GA as We,hp as Fe,tQ as Ge,hr as Be,hs as He,ht as ke,hw as Qe,p as M,u as Xe,GB as Je,dC as B}from"./index-BCSI5g4r.js";import{t as Ke}from"./VisualElement-4b3ubyV7.js";let H=class{constructor(e){this._renderCoordsHelper=e,this._origin=o(),this._dirty=!1,this._count=0}set vertices(e){const n=pe(3*e.length);let i=0;for(const s of e)n[i++]=s[0],n[i++]=s[1],n[i++]=s[2];this.buffers=[n]}set buffers(e){if(this._buffers=e,this._buffers.length>0){const n=this._buffers[0],i=3*Math.floor(n.length/3/2);T(this._origin,n[i],n[i+1],n[i+2])}else T(this._origin,0,0,0);this._dirty=!0}get origin(){return this._origin}draw(e){const n=this._ensureVAO(e);n!=null&&(e.bindVAO(n),e.drawArrays(ue.TRIANGLES,0,this._count))}dispose(){this._vao!=null&&this._vao.dispose()}get _layout(){return this._renderCoordsHelper.viewingMode===1?Ze:et}_ensureVAO(e){return this._buffers==null?null:(this._vao??=this._createVAO(e,this._buffers),this._ensureVertexData(this._vao,this._buffers),this._vao)}_createVAO(e,n){const i=this._createDataBuffer(n);return this._dirty=!1,new fe(e,new ge(e,_e(this._layout),i))}_ensureVertexData(e,n){if(!this._dirty)return;const i=this._createDataBuffer(n);e.buffer()?.setData(i),this._dirty=!1}_createDataBuffer(e){const n=e.reduce((g,c)=>g+k(c),0);this._count=n;const i=this._layout.createBuffer(n),s=this._origin;let a=0,l=0;const d="startUp"in i?this._setUpVectors.bind(this,i):void 0;for(const g of e){for(let c=0;c<g.length;c+=3){const L=T(Q,g[c],g[c+1],g[c+2]);c===0?l=this._renderCoordsHelper.getAltitude(L):this._renderCoordsHelper.setAltitude(L,l);const _=a+2*c;d?.(c,_,g,L);const j=J(Q,L,s);if(c<g.length-3){for(let P=0;P<6;P++)i.start.setVec(_+P,j);i.extrude.setValues(_,0,-1),i.extrude.setValues(_+1,1,-1),i.extrude.setValues(_+2,1,1),i.extrude.setValues(_+3,0,-1),i.extrude.setValues(_+4,1,1),i.extrude.setValues(_+5,0,1)}if(c>0)for(let P=-6;P<0;P++)i.end.setVec(_+P,j)}a+=k(g)}return i.buffer}_setUpVectors(e,n,i,s,a){const l=this._renderCoordsHelper.worldUpAtPosition(a,Ye);if(n<s.length-3)for(let d=0;d<6;d++)e.startUp.setVec(i+d,l);if(n>0)for(let d=-6;d<0;d++)e.endUp.setVec(i+d,l)}};function k(t){return 3*(2*(t.length/3-1))}const Ye=o(),Q=o(),Ze=K().vec3f("start").vec3f("end").vec2f("extrude").vec3f("startUp").vec3f("endUp"),et=K().vec3f("start").vec3f("end").vec2f("extrude");function oe(t,e){const n=t.fragment;n.include(me),t.include(ve),n.include(Pe),n.uniforms.add(new f("globalAlpha",i=>i.globalAlpha),new w("glowColor",i=>i.glowColor),new f("glowWidth",(i,s)=>i.glowWidth*s.camera.pixelRatio),new f("glowFalloff",i=>i.glowFalloff),new w("innerColor",i=>i.innerColor),new f("innerWidth",(i,s)=>i.innerWidth*s.camera.pixelRatio),new be("depthMap",i=>i.depth?.attachment),new z("normalMap",i=>i.normals)),n.code.add(r`vec4 premultipliedColor(vec3 rgb, float alpha) {
